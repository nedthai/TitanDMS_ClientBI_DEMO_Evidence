---------------------------------------------------------------------------------------------------------------
-- 0. SCHEMA DESCRIPTION | + To describe the purpose of this cript and the schema of database 'DemoDealerAI' --
---------------------------------------------------------------------------------------------------------------

-- PURPOSE OF THIS SCRIPT
--
-- + This script is designed to build up the SQL Server database 'DemoDealerAI' by | * creating necessary tables
...
-- THE SCHEMA OF DATABASE 'DemoDealerAI'
...
--   - dbo.DimMake is the father of dbo.DimModelType which is the father of dbo.DimModel abd dbo.DimVehicle is the child of dbo.DimModel
...
CREATE TABLE dbo.DimCompany (
    Company_Key       INT,
    Company_Fullname  NVARCHAR(256),
    Company_Shortname NVARCHAR(256)
)
...
CREATE TABLE dbo.DimLocation (
    Location_ID        INT,
    Location_Fullname  NVARCHAR(256),
    Location_Shortname NVARCHAR(256),
    Location_Fullinfo  NVARCHAR(500),
    Is_Active          BIT,
    Company_Key        INT
)
...
CREATE TABLE dbo.DimModelType (
    ...
    Model_Type_Description NVARCHAR(512),
    Is_Active              BIT,
    Make_Key               INT
)
...
CREATE TABLE dbo.DimModel (
    ...
    Model_Description NVARCHAR(1200),
    Model_Series      NVARCHAR(80),
    Model_Badge       NVARCHAR(100),
    Is_Active         BIT,
    Model_Type_Key    INT
)
...
SELECT COMPANY_KEY  AS Company_Key,
       COMPANY_NAME AS Company_Fullname,
       COMPANY_NAME AS Company_Shortname
...
SELECT LOCATION_ID   AS Location_ID,
       LOCATION_NAME AS Location_Fullname,
       LOCATION_NAME AS Location_Shortname,
       LOCATION_NAME AS Location_Fullinfo,
...
SELECT MAKE_KEY         AS Make_Key,
       ...
       MAKE_DESCRIPTION AS Make_Description,
       IS_ACTIVE        AS Is_Active
...
SELECT MODEL_TYPE_KEY         AS Model_Type_Key,
       ...
       MODEL_TYPE_DESCRIPTION AS Model_Type_Description,
       IS_ACTIVE              AS Is_Active,
...
SELECT MODEL_KEY         AS Model_Key,
       ...
       MODEL_SERIES      AS Model_Series,
       MODEL_BADGE       AS Model_Badge,
       IS_ACTIVE         AS Is_Active,
...
SELECT -2                AS Model_Key,
       ...
       'USED VEHICLE'    AS Model_Description,
       NULL              AS Model_Series,
       NULL              AS Model_Badge,
       1                 AS Is_Active,
       1                 AS Model_Type_Key
...
UPDATE c
SET c.Company_Fullname  = 'Company ' + CAST(Company_Key AS VARCHAR(10)),
    c.Company_Shortname = 'Company ' + CAST(Company_Key AS VARCHAR(10))
--SELECT c.*
FROM dbo.DimCompany c

UPDATE l
SET l.Location_Fullname  = 'Location ' + CAST(Location_ID AS VARCHAR(10)),
    l.Location_Shortname = 'Location ' + CAST(Location_ID AS VARCHAR(10)),
    l.Location_Fullinfo  = 'Location ' + CAST(Location_ID AS VARCHAR(10))
--SELECT l.*
